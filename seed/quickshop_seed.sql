...
-- Orders (two ~last month, two this week), then items
DECLARE @o1 BIGINT, @o2 BIGINT, @o3 BIGINT, @o4 BIGINT;

INSERT INTO dbo.orders (customer_id, created_at, status)
VALUES (1, DATEADD(DAY, -35, SYSUTCDATETIME()), 'completed');  -- last month
SET @o1 = SCOPE_IDENTITY();

INSERT INTO dbo.orders (customer_id, created_at, status)
VALUES (2, DATEADD(DAY, -34, SYSUTCDATETIME()), 'completed');  -- last month
SET @o2 = SCOPE_IDENTITY();

INSERT INTO dbo.orders (customer_id, created_at, status)
VALUES (2, DATEADD(DAY,  -3, SYSUTCDATETIME()), 'completed');  -- this week
SET @o3 = SCOPE_IDENTITY();

INSERT INTO dbo.orders (customer_id, created_at, status)
VALUES (3, DATEADD(DAY,  -2, SYSUTCDATETIME()), 'completed');  -- this week
